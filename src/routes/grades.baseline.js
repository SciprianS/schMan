...
// Decomenteza in app.js: app.use('/api/baseline/grades', require('./routes/grades.baseline'));
...
router.get('/', authenticate, async (req, res) => {
  const result = await db.query('SELECT * FROM grades ORDER BY created_at DESC');
  res.json(result.rows);
});

router.get('/:id', authenticate, async (req, res) => {
  const result = await db.query('SELECT * FROM grades WHERE id = $1', [req.params.id]);
  if (!result.rows.length) return res.status(404).json({ error: 'Nota nu exista.' });
