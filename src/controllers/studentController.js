  ...
  try {
    ...
    if (req.ability.cannot('read', studentSubject)) {
      await logAction({
        ...
        action: 'READ', resourceType: 'Student',
...
async function create(req, res) {
  try {
    const { userId, className, year, parentName, parentPhone, birthDate } = req.body;

    if (!userId || !className || !year) {
      return res.status(400).json({ error: 'userId, className si year sunt obligatorii.' });
    }

    const result = await db.query(
      `INSERT INTO students (user_id, class_name, year, parent_name, parent_phone, birth_date)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [userId, className, year, parentName || null, parentPhone || null, birthDate || null]
    );

    await logAction({
      userId: req.user.id, userRole: req.user.role,
      action: 'CREATE', resourceType: 'Student',
      resourceId: result.rows[0].id, newValue: result.rows[0],
      ipAddress: req.ip,
    });
