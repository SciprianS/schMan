...
async function register(req, res) {
  try {
    ...
    const existent = await db.query("SELECT id FROM users WHERE email = $1", [
      email,
    ]);
    ...
    // Creeaza automat profil de elev daca rolul este student
    if (role === "student") {
      await db.query(
        `INSERT INTO students (user_id, class_name, year)
         VALUES ($1, 'Neatribuită', EXTRACT(YEAR FROM NOW()))`,
        [newUser.id],
      );
    }
...
async function login(req, res) {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ error: "Email si parola sunt obligatorii." });
    }
