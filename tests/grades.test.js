...
beforeAll(async () => {
  await seedTestData();
  tokens = getTokens(); // acum stie ID-urile reale
});
afterAll(async () => {
  await clearTestData();
});
...
describe('POST /api/grades', () => {
  test('Profesorul poate crea o nota', async () => {
    ...
    const res = await request(app)
      .post('/api/grades')
      .set('Authorization', `Bearer ${tokens.prof1}`)
      .send({ studentId: IDS.stud1, courseId: IDS.course1, value: 9.5 });
